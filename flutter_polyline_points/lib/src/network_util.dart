...
import 'utils/geocode_result.dart';
...
class NetworkUtil {
  ...
  Future<GeocodingResult?> getAddressByCoordinates(
    ...
    try {
      var params = {"latlng": "$latitude,$longitude", "key": googleKey};
      Uri uri =
      ...
      if (Platform.isIOS) {
        headers.addAll({"x-ios-bundle-identifier": bundleId ?? ""});
      } else {
        headers.addAll({
      ...
      if (response.statusCode == 200) {
        ...
        if (parsedJson["results"] is List && parsedJson["results"].length > 0) {
          final resultData =
              GeocodingResult.fromJson((parsedJson["results"] as List).first);
          return resultData;
        }
      }
      return null;
  ...
  Future<PolylineResult> getRouteBetweenCoordinates(
    ...
    var response = await http.get(uri, headers: headers);
    log("flutter_polyline_points: ${response.headers}");
    if (response.statusCode == 200) {
      var parsedJson = json.decode(response.body);
